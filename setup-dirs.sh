...
rm    -rf src install
mkdir -pv src install

for version in ${versions}; do
  advantg_dir=/opt/software_misc/ADVANTG-${version}
  ...
  for src_type in ${src_types}; do
    ...
    cd ${install_dir}
    ln -sv ${base_dir}/src/${version}-${src_type} advantg
  done

  # Fix advantg.rc
  if [ ${version} == "3.0.3" ]; then continue; fi
  for src_type in ${src_types}; do
    cd ${base_dir}/install/${version}-${src_type}
    rm -fv advantg.rc
    cp -pv ${advantg_dir}/advantg.rc .
    dir1=`echo ${advantg_dir}                             | sed 's/\//\\\\\//g'`
    dir2=`echo ${base_dir}/install/${version}-${src_type} | sed 's/\//\\\\\//g'`
